...
public class TodoReadController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            // findCookie() 메소드는 저장되어있는 Cookie중 viewTodoCookie가 있는지 확인하는 메소드
            Cookie viewTodoCookie = findCookie(req.getCookies(), "viewTodoCookie");
            String todoListStr = viewTodoCookie.getValue();
            boolean exist = false;

            // 눌렀던 tno를 저장하는 처리
            // 쿠키의 값이 있는지 없는지 확인 && tno 가 쿠키가 값 안에 존재하는지 확인 하는 if문
            // 새로운 데이터만 저장되고 이미 이쓴ㄴ 데이터는 한 번 더 저장되지 않도록 하는 부분
            if(todoListStr != null && todoListStr.indexOf(tno+"-")>= 0){
                exist = true;
            }
            if(!exist){
                // 기존에 저장되어 있던 기록에 tno를 더함
                todoListStr += tno+"-";
    ...
    public Cookie findCookie(Cookie[] cookies, String cookieName) {
        ...
        // cookies가 null 이 아니고 데이터가 0개보다 클 때
        if (cookies != null && cookies.length > 0) {
            // cookies에 들어있는 쿠키의 이름을 확인하기 위한 반복문
            for (Cookie cookie : cookies) {
                // cookie의 key값이 찾는 이름과 같은 지 확인하는 if문
        ...
        // 이미 만들어진 쿠키가 없을 때 새로운 쿠키를 생성하는 if문
        if (targetCookie == null) {
            // cookieName으로 쿠키를 생성
            targetCookie = new Cookie(cookieName, "");
            // 경로 설정 / = localhost:8800
            targetCookie.setPath("/");
            // 만료기간 설정 : 초단위로 설정하기 때문에 60초*60*24 = 1일
