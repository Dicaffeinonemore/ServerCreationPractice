...
public class TodoDAO {
    public String getTime(){
        String now = null;
        // try with resource 문 : Connection, PreparedStatement, ResultSet 은 close()를 실행해야하기 때문에 사용
        try (Connection connection = ConnectionUtil.INSTANCE.getConnection();
             // PreparedStatement : SQL 실행하기 위한 사전준비
             PreparedStatement pstmt = connection.prepareStatement("SELECT now()");
             // ResultSet : PreparedStatement 에 설정한 SQL 의 실행결과를 저장하는 객체
             ResultSet rs = pstmt.executeQuery()) {
            //iterator 의 특징을 가진 객체로 next() 메소드를 이용하여 데이터를 출력 가능
            rs.next();// 첫번째 데이터로 이동
        ...
        // @Cleanup : lombok에서 지원하는 어노테이션으로 close 를 실행해주는 기능
    ...
    public List<TodoVO> selectAll() throws Exception {
        ...
        while (rs.next()){
            // List<TodoVO> 안에 담기위한 TodoVO 객체 생성
            TodoVO vo = TodoVO.builder()
                ...
                .build();
            // List<TodoVO>에 vo 데이터 추가
            list.add(vo);
        }
        return list;
    }
    public TodoVO selectOne(Long tno) throws Exception{
        // 데이터 한 개를 취득하는 경우 Primary Key 를 ㅣ용하여 검색하는 것이 가장 빠르다.
        String sql = "SELECT * FROM tbl_todo WHERE tno = ?";
        @Cleanup Connection connection = ConnectionUtil.INSTANCE.getConnection();
        @Cleanup PreparedStatement pstmt = connection.prepareStatement(sql);
        // ?의 데이터를 반드시 설정해야함
        pstmt.setLong(1, tno);
        // SELECT 문을 실행 후 결과를 ResultSet 에 저장
        @Cleanup ResultSet rs = pstmt.executeQuery();
        // 첫번째 데이터를 꺼내는 기능
        rs.next();
    ...
    public TodoVO delete(Long tno) throws Exception{
