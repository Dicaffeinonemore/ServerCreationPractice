...
public class LoginCheckFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        // 쿠키에 UUID가 있으면 로그인이 되어야 하기 때문에 JSESSIONID 확인 불필요
//        if(session.isNew()){
//            response.sendRedirect("/login");
//            return;
//        }
        // 세션에 로그인 정보가 있을 경우 다음 페이지로 진행하돌고 설정하는 if문
        ...
        try {
            ...
            //uuid를 이용하여 데이터베이스의 Member 데이터 취득
            MemberDTO dto = MemberService.INSTANCE.getByUUID(uuid);
            // 데이터베이스에 해당하는 UUID가 없을 겨웅 예외를 ㅂ발생시키고 로그인 페이지로 이동
            if (dto == null) {
    ...
    private Cookie findCookie(Cookie[] cookies, String name) {
        // 쿠키가 하나도 없으면 null 값을 반환 , 로그인 관련 처리이기 때문에 새로운 쿠키를 만들지 않음
        if (cookies == null || cookies.length == 0) {
            return null;
        }
        Optional<Cookie> result = Arrays.stream(cookies)
                // filter(cookies의 데이터 한개 -> 조건식) : 리스트나 배열 안의 데이터 즁 조건식에 맞는 데이터만 남겨줌
